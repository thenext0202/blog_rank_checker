"""
심의안전 원고제작기 — 서식 파싱 + Word 출력
기존 원고제작기의 parse_annotation / save_as_docx 재사용
"""
import re
...
def _build_styled_segments(original_text, colored_words):
    """텍스트를 마크다운 볼드/이탤릭 + 색상 기준으로 세그먼트 분할"""
    md_re = re.compile(r'\*\*\*(.+?)\*\*\*|\*\*(.+?)\*\*|\*(.+?)\*')
    chunks = []
    last = 0
