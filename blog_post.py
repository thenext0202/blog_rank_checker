...
class NaverBlogPoster:
    ...
    def navigate_to_editor(self):
        ...
        try:
            ...
            print("  mainFrame 없음 - 현재 페이지에서 에디터 탐색")
    ...
    def publish(self):
        ...
        pub_selectors = [
            "button.publish_btn__Y5mLP",
            "button.publish_btn",
            "//button[contains(text(), '발행')]",
        ]
        pub_btn = find_element_by_selectors(self.driver, pub_selectors, wait=10)
        if not pub_btn:
        ...
        # 발행 확인 다이얼로그
        confirm_selectors = [
            "button.confirm_btn__Dv9du",
            "button.confirm_btn",
            "//button[contains(text(), '확인')]",
        ]
        confirm_btn = find_element_by_selectors(self.driver, confirm_selectors, wait=5)
        if confirm_btn:
            confirm_btn.click()
            time.sleep(3)
            print("  발행 완료!")
        ...
        print("  [!] 발행 확인 버튼을 찾지 못했습니다 (수동 확인 필요)")
        return False
